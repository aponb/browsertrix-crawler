...
describe("domain attribution", () => {
  ...
  test("keeps the originating seedId when discovered links are queued", async () => {
    ...
    await crawler.queueInScopeUrls({
      seedId: 7,
      urls: ["https://cdn.example.org/embed", "https://www.foo.bar/page"],
      depth: 2,
      extraHops: 1,
    });

    expect(queueUrl).toHaveBeenNthCalledWith(1, {
      seedId: 7,
      url: "https://cdn.example.org/embed",
      depth: 3,
      extraHops: 1,
    ...
    expect(queueUrl).toHaveBeenNthCalledWith(2, {
      seedId: 7,
      url: "https://www.foo.bar/page",
      depth: 3,
      extraHops: 2,
